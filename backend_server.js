...
// JWT_SECRET loading with file support (Docker Secrets pattern)
let JWT_SECRET;

if (process.env.JWT_SECRET_FILE) {
  try {
    console.log('🔐 Loading JWT_SECRET from file...');
    JWT_SECRET = fs.readFileSync(process.env.JWT_SECRET_FILE, 'utf8').trim();
    console.log('✅ JWT_SECRET loaded from file successfully');
  } catch (error) {
    console.error(`💥 ERROR: Failed to read JWT_SECRET from file ${process.env.JWT_SECRET_FILE}:`, error.message);
...
} else if (process.env.JWT_SECRET && process.env.JWT_SECRET.trim() !== '') {
  JWT_SECRET = process.env.JWT_SECRET;
  ...
  console.error('  - JWT_SECRET_FILE environment variable pointing to secret file');
...
app.get('/api/auth/registration-status', async (req, res) => {
  try {
    ...
    res.json({ allowed: setting?.setting_value === 'true' });
  } catch (error) {
    console.error('Error checking registration status:', error);
    res.json({ allowed: false }); // Default to false on error
  }
});
  ...
  try {
    await database.run(
      `INSERT OR REPLACE INTO system_settings (setting_key, setting_value, updated_at) 
       VALUES ('allow_registration', ?, datetime('now'))`,
